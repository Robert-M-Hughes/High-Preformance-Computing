...
double * gaussianKernel(double sigma){
  //printf("Number of threads in the Gaussian: %d\n", omp_get_num_threads());
    ...
    gaussiankernel = (double*)malloc(w*sizeof(double)); // since we know the width of the array we can statically declare it
...
double * gaussianDerivKernel( double sigma){
    ...
    float w; // the width of the kernel
    float sum = 0; // set the intial sum
    double* gaussianDeriv;
    a = round(2.5*sigma-.5);//calculate and round the value of a
    w = 2*a+1; // calculate the width of the kernel
    gaussianDeriv = (double*)malloc(w*sizeof(double));//can statically allocate since we know the width
    int i;
    for(i = 0; i < w; i++){
